...
\begin{figure*}[hbt!]
	...
	\caption{Orthogonal views of the three galaxies (disk particles only) at first apocenter.
...
More intuitive results were obtained once the $\hat{L}$ calculation was limited to stars within a limited radius of the CoM.  Figure \ref{fig:rem_shape} uses a 40 kpc limit and shows that the major axis is approximately parallel to the rotation axis.
...
In this inner region the stellar remnant is near-prolate. Figure \ref{fig:isophote_xy} shows the contour points from the left panel of Figure \ref{fig:rem_shape}, overlaid with an ellipse (solid line) that is an approximate best fit. Ellipticity is low in this midplane, around 0.15.
...
Figure \ref{fig:isophote_xy} shows a similar fit for the mid panel of Figure \ref{fig:rem_shape}. Ellipticity is much higher, around 0.5. The inner contour is also waisted and somewhat boxy; the outer contour appears more regular, but a more detailed statistical analysis is needed.
