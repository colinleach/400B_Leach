...
More intuitive results were obtained once the $\hat{L}$ calculation was limited to stars within a constrained radius of the CoM.  Figure \ref{fig:rem_shape} uses a 40 kpc limit and shows that the major axis is approximately parallel to the rotation axis.
...
In this inner region the stellar remnant is near-prolate. Figure \ref{fig:isophote_xy} shows the contour points from the left panel of Figure \ref{fig:rem_shape}, overlaid with an ellipse (solid line) that is an approximate best fit. Ellipticity $\epsilon$ is low in this midplane, around 0.15.
...
Figure \ref{fig:isophote_xy} shows a similar fit for the mid panel of Figure \ref{fig:rem_shape}. Ellipticity is much higher, around 0.5. The inner contour is also waisted and somewhat boxy; the outer contour appears more regular, but further statistical analysis is needed.
...
There may be a trend for the ellipticity to fall between the 50\% and 75\% contours, but the data so far is not convincing. If we add a 90\% contour (not shown in Figure \ref{fig:rem_shape}), the picture changes significantly. Out at radii of 20-30 kpc, the overall shape is more oblate, with the short axis closer to $\hat{x}$ as shown in Figure \ref{fig:isophotes_90}. 

\begin{figure}[htb!]
	\epsscale{1.2}
	\plotone{isophotes_90.pdf}
	\caption{Contour fitting for the 90\% contour. Coordinates as in Figures \ref{fig:rem_shape} to \ref{fig:isophote_xz}.
