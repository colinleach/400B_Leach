...
\todo{\textbf{remnant DM $\sigma$, virial mass}}
...
The DM halo obviously has no sharp outer edge, it just gradually fades into the inter-galactic medium. One popular convention is to use the $r_{200}$ or ``virial radius'' as a limit: the radius at which the density is $200\times$ the cosmological critical density $\rho_c$.

For a flat LambdaCDM cosmology, we can calculate the critical density from
\[ \rho_c(t) = \frac{3 H^2(t)}{8 \pi G} \] 
Currently \citep{planck_collaboration_planck_2016} we have \\
...
This gives us a value of $\rho_{c,\infty} \approx 90\ M_\sun/\text{kpc}^3$. Then $r_{200}$ is the radius at which the spherically-averaged remnant density falls below $1.8 \times 10^4\ M_\sun/\text{kpc}^3$. At the final timepoint, this is 
\[ r_{200} \approx 170\ \text{kpc} \]
