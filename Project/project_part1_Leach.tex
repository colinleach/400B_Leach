...
%Meanwhile, here is some nonsense:
%
%\lipsum[1-1]
...
% The next bit is commented out for performance reasons. These pdf files are so complicated to render that it's much quicker to make it once, take a screen grab and then use that png file

%\begin{figure*}[!bht]
%	\gridline{\fig{cyl_MW_000.pdf}{0.3\textwidth}{}
%		\fig{cyl_MW_278.pdf}{0.3\textwidth}{}
%		\fig{cyl_MW_300.pdf}{0.3\textwidth}{}}
%	\gridline{\fig{cyl_M31_000.pdf}{0.3\textwidth}{Start}
%		\fig{cyl_M31_278.pdf}{0.3\textwidth}{First pericenter}
%		\fig{cyl_M31_300.pdf}{0.3\textwidth}{Near apocenter}}
...
The MW and M31 disks have angular momentum vectors inclined at an angle of $52^\circ$ to each other shortly before pericenter.

\todo{Clarify prograde/retrograde!}
...
%\end{deluxetable}
%
%The mutual inclination angles of the major axes are shown in Table 5. Again, the former disk particles are seen to retain a distinct memory of their origin. Clearly this collision and a single merger is not sufficient to randomize stars within the remnant \todo{refs?}. Also, this is consistent with the understanding that relaxation times are very long in collisionless systems on the scale of elliptical galaxies \citep[Section 1.2]{binney_galactic_2008}
%
...
The mass profile for each type of particle and overall is shown in Figure \ref{fig:rem_mp_type}. This is similar to Figure \ref{fig:massprof0}, except that the radius now extends out to 100 kpc.
...
The virial mass, enclosed within the virial radius, is $1.7 \times 10^{12}\ M_\Sun$, of which 87\% is Dark Matter.
...
\subsubsection{Remnant angular momentum}
...
\begin{figure}[htb!]
	\epsscale{0.9}
	\plotone{rem_h_r.pdf}
	\caption{Ex-disk star specific angular momentum of the MW-M31 remnant within various radii ($h$ has arbitrary units).
		\label{fig:rem_h_r}}
\end{figure}

That analysis used all particles origination from the MW and M31, regardless of distance from the CoM. To check the validity of this, we calculated how specific angular momentum varies with radius, Attempting to do this for thin spherical shells gave surprisingly noisy results with no clear interpretation. Instead, Figure \ref{fig:rem_h_r} shows values for all ex-disk stars within various radii of the CoM. The profile is unexpectedly complex, with stars inside 50 kpc apparently quite distinct from those outside 70 kpc.

\begin{figure}[htb!]
	\epsscale{0.9}
	\plotone{rem_phi_theta.pdf}
	\caption{Ex-disk star angular momentum orientation of the MW-M31 remnant within various radii (spherical coordinates).
		\label{fig:rem_phi_theta}}
\end{figure}

To investigate this further, Figure \ref{fig:rem_phi_theta} shows the orientation of $\hat{L}(r)$ in spherical coordinates, where $\phi$ is the azimuthal angle in the $x,y$ plane and $\theta$ is the polar angle downwards from the positive $z$-axis, as in Figure \ref{fig:sph_coord}. Again, we see a break in the 50-70 kpc radius range. With the major caveat that we are looking at spherically-averaged values for a structure which probably has substantial (but at this point undefined) ellipticity, the remnant is clearly far from equilibrium and has a complex structure and kinematics  
...
More intuitive results were obtained once the $\hat{L}$ calculation was limited to stars within a limited radius of the CoM.  Figure \ref{fig:rem_shape} uses a 50 kpc limit and shows that the major axis is approximately parallel to the rotation axis. Centering is clearly not perfect \todo{can this be improved?}

\begin{figure}[htb!]
	\epsscale{1.1}
	\plotone{remnant_shape.pdf}
	\caption{Luminous star density of the MW-M31 remnant in three orthogonal projections. Left panel looks down the $\hat{L}$ axis, mid/right panels have this vertical.
		\label{fig:rem_shape}}
\end{figure}
...
Mean radial velocities $\bar{v}$ and velocity dispersions $\sigma$ were calculated by binning along the $y$-axis (Figure \ref{fig:rem_disp_y}) and the $z$-axis (Figure \ref{fig:rem_disp_z}). The results are not entirely simple and regular, but the variation perpendicular to $\hat{L}$ shows clear rotation with $v_{\text{max}} \approx 83$ km/s. As expected, along $\hat{L}$ we see smaller and more random velocities with a somewhat higher dispersion.

\begin{figure}[htb!]
	\epsscale{0.9}
	\plotone{remnant_stellar_disp_y.pdf}
...
\begin{figure}[htb!]
	\epsscale{0.9}
	\plotone{remnant_stellar_disp_z.pdf}
