...
\begin{align*}
	\hat{v}_a &= \left< -0.114, -0.416 , +0.902 \right> \\
	\hat{v}_b &= \left< -0.524, +0.797,  +0.302 \right> \\
	\hat{v}_c &= \left< -0.844, -0.438, -0.309 \right>
\end{align*}
...
\todo{Would be better to plot $\rho(r)$}
...
A previous section showed that the remnant is elliptical and particles of different orientation are oriented very differently (Table \ref{tbl:remn_axis_angles}). When this analysis is repeated for the angular momentum vectors the mutual inclination angles are non-zero but generally quite small, as shown in Table \ref{tbl:remn_rot_angle}.
...
The author is grateful to Professor Gurtina Besla for teaching the class on which this paper is based and for allowing this rather geriatric student to participate, as well as providing all the raw data from the earlier simulation described in vdM12. Also to Rixin Li for valuable coding advice.
